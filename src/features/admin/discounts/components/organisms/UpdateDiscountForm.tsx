...
export function UpdateDiscountForm({ discount, courses }: UpdateDiscountFormProps) {
    const {
        formData,
        isSubmitting,
        handleChange,
        ...
        handleSubmit,
        setFormData
    } = useUpdateDiscount(discount.id, discount)

    return (
        <form onSubmit={handleSubmit} className="mx-auto max-w-7xl space-y-8" dir="rtl">
            <AdminPageHeader
                title={`تعديل الخصم: ${discount.name}`}
                subtitle="تحديث تفاصيل الخصم وفترة الصلاحية والكورسات المستهدفة"
                breadcrumbs={[
                    { label: "الخصومات", href: "/discounts" },
                    { label: discount.name, href: `/discounts/${discount.id}` },
                    { label: "تعديل" }
                ]}
                actions={
                    <div className="flex items-center gap-2">
                        <Button variant="outline" type="button" onClick={() => window.history.back()} className="h-11 px-6 border-2 font-bold rounded-xl hover:bg-muted/50 transition-all">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="border rounded-2xl bg-card shadow-sm overflow-hidden border-primary/10">
                        ...
                        <CardContent className="p-8 space-y-6">
                                ...
                                <div className="flex gap-4">
                                    ...
                                    <label className="flex items-center gap-2 cursor-pointer p-3 border rounded-xl hover:bg-muted/50 transition-colors flex-1">
                                        <input
                                            type="radio"
                                            className="accent-primary size-4"
                                            checked={formData.appliesTo === "COURSE" || formData.appliesTo === "COURSES" as any}
                            ...
                            {(formData.appliesTo === "COURSE" || formData.appliesTo === "COURSES" as any) && (
